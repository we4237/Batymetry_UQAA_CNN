...
#     print('测试集R^2: %f' % regrSVR.score(Xtest, Ytest))
#     print('RMSE: %f' % mean_squared_error(Y_SVRtest, Ytest) ** 0.5)
#     RMSE_UQAA.append(mean_squared_error(Y_SVRtest, Ytest) ** 0.5)
#     print('MAPE: %f' % (mean_absolute_percentage_error(Y_SVRtest, Ytest)*100))
#     MRE_UQAA.append((mean_absolute_percentage_error(Y_SVRtest,Ytest)*100))
...
#     print('测试集R^2: %f' % regrSVR.score(Xtest, Ytest))
#     print('RMSE: %f' % mean_squared_error(Y_SVRtest, Ytest) ** 0.5)
#     RMSE_noUQAA.append(mean_squared_error(Y_SVRtest, Ytest) ** 0.5)
#     print('MAPE: %f' % (mean_absolute_percentage_error(Y_SVRtest, Ytest)*100))
#     MRE_noUQAA.append((mean_absolute_percentage_error(Y_SVRtest,Ytest)*100))

# RMSE_UQAA = np.array(RMSE_UQAA)
# np.save('RMSE_svr_UQAA.npy',RMSE_UQAA)
# RMSE_npUQAA = np.array(RMSE_noUQAA)
# np.save('RMSE_svr_noUQAA.npy',RMSE_npUQAA)
RMSE_UQAA = np.load('RMSE_svr_UQAA.npy')
RMSE_noUQAA = np.load('RMSE_svr_noUQAA.npy')
# RMSE折线图
plt.figure(figsize=(6, 4), dpi=80)
plt.plot(size, RMSE_UQAA, color='blue', label='SVR with UQAA')
plt.plot(size, RMSE_noUQAA, color='orange', label='SVR without UQAA')
...
plt.ylim(1, 1.5)  # 设置y坐标轴的显示范围

plt.savefig('SVR_RMSE' + '.svg', bbox_inches='tight', dpi=1000)
plt.show()

# MRE_UQAA = np.array(MRE_UQAA)
# np.save('MRE_svr_UQAA.npy',MRE_UQAA)
# MRE_npUQAA = np.array(MRE_noUQAA)
# np.save('MRE_svr_noUQAA.npy',MRE_npUQAA)
MRE_UQAA = np.load('MRE_svr_UQAA.npy')
MRE_noUQAA = np.load('MRE_svr_noUQAA.npy')
# MRE折线图
plt.figure(figsize=(6, 4), dpi=80)
plt.plot(size, MRE_UQAA, color='blue', label='SVR with UQAA')
plt.plot(size, MRE_noUQAA, color='orange', label='SVR without UQAA')
font1 = {'family':'Times New Roman','weight':'heavy','size': 14,}
plt.legend(loc='upper right',prop = font1)  # 显示图例
plt.ylabel("MRE(%)", family='Times New Roman',weight = 'heavy',fontsize=20)
plt.xticks(range(0,len(size),2),fontproperties='Times New Roman',fontsize=14)
plt.yticks(fontproperties='Times New Roman',fontsize=14)
plt.ylim(13, 23)  # 设置y坐标轴的显示范围

plt.savefig('SVR_MRE' + '.svg', bbox_inches='tight', dpi=1000)
plt.show()