...
#
#     # XGBoost
#     print('=' * 10, 'XGBoost',str(i) ,'=' * 10)
#     regrXG = XGBRegressor(learning_rate=0.05, eta=0.01, max_depth=3, n_estimators=2000, random_state=0)
#     regrXG.fit(Xtrain, Ytrain)
#     Yhat_XG = regrXG.predict(Xtrain)
#     Y_XGtest = regrXG.predict(Xtest)
#     print('训练集R^2: %f' % regrXG.score(Xtrain, Ytrain))
#     print('测试集R^2: %f' % regrXG.score(Xtest, Ytest))
#     print('RMSE: %f' % mean_squared_error(Y_XGtest, Ytest) ** 0.5)
#     RMSE_UQAA.append(mean_squared_error(Y_XGtest, Ytest) ** 0.5)
#     print('MAPE: %f' % mean_absolute_percentage_error(Y_XGtest, Ytest))
#     MRE_UQAA.append((mean_absolute_percentage_error(Y_XGtest, Ytest) * 100))
#
#
# RMSE_noUQAA = []
# MRE_noUQAA = []
...
#     print('测试集R^2: %f' % regrXG.score(Xtest, Ytest))
#     print('RMSE: %f' % mean_squared_error(Y_XGtest, Ytest) ** 0.5)
#     RMSE_noUQAA.append(mean_squared_error(Y_XGtest, Ytest) ** 0.5)
#     print('MAPE: %f' % mean_absolute_percentage_error(Y_XGtest, Ytest))
#     MRE_noUQAA.append((mean_absolute_percentage_error(Y_XGtest, Ytest) * 100))

# RMSE_UQAA = np.array(RMSE_UQAA)
# np.save('RMSE_XG_UQAA.npy',RMSE_UQAA)
# RMSE_npUQAA = np.array(RMSE_noUQAA)
# np.save('RMSE_XG_noUQAA.npy',RMSE_npUQAA)
RMSE_UQAA = np.load('RMSE_XG_UQAA.npy')
RMSE_noUQAA = np.load('RMSE_XG_noUQAA.npy')
# RMSE折线图
plt.figure(figsize=(6, 4), dpi=80)
plt.plot(size, RMSE_UQAA, color='blue', label='XGBoost with UQAA')
plt.plot(size, RMSE_noUQAA, color='orange', label='XGBoost without UQAA')
...
plt.savefig('XG_RMSE' + '.svg', bbox_inches='tight', dpi=1000)
plt.show()

# MRE_UQAA = np.array(MRE_UQAA)
# np.save('MRE_XG_UQAA.npy',MRE_UQAA)
# MRE_npUQAA = np.array(MRE_noUQAA)
# np.save('MRE_XG_noUQAA.npy',MRE_npUQAA)
MRE_UQAA = np.load('MRE_XG_UQAA.npy')
MRE_noUQAA = np.load('MRE_XG_noUQAA.npy')
# MRE折线图
plt.figure(figsize=(6, 4), dpi=80)
plt.plot(size, MRE_UQAA, color='blue', label='XGBoost with UQAA')
plt.plot(size, MRE_noUQAA, color='orange', label='XGBoost without UQAA')
font1 = {'family':'Times New Roman','weight':'heavy','size': 14,}
plt.legend(loc='upper right',prop = font1)  # 显示图例
...
plt.savefig('XG_MRE' + '.svg', bbox_inches='tight', dpi=1000)
plt.show()